...
export class MsgClientImpl {
    ...
    CreateGame(request) {
        ...
        const promise = this.rpc.request("zjing20.checkers.checkers.Msg", "CreateGame", data);
        return promise.then((data) => MsgCreateGameResponse.decode(new Reader(data)));
    }
    PlayMove(request) {
        const data = MsgPlayMove.encode(request).finish();
        const promise = this.rpc.request("zjing20.checkers.checkers.Msg", "PlayMove", data);
        return promise.then((data) => MsgPlayMoveResponse.decode(new Reader(data)));
    }
    RejectGame(request) {
        const data = MsgRejectGame.encode(request).finish();
        const promise = this.rpc.request("zjing20.checkers.checkers.Msg", "RejectGame", data);
